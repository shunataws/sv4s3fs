       ...
       string canonical_headers, string payload_hash, string signed_headers, string date3)
{
  ...
  unsigned int i;
  s3fs_HMAC(kDate, kDate_len, (unsigned char *)"cn-north-1", sizeof("cn-north-1")-1, &kRegion, &kRegion_len);
  ...
  StringToSign += date3+"\n";
  StringToSign += date2+"/cn-north-1/s3/aws4_request\n";
...
int S3fsCurl::DeleteRequest(const char* tpath)
{
  ...
  string canonical_headers = "host:" + bucket + "." + endpoint + "\nx-amz-content-sha256:" \
    + payload_hash + "\nx-amz-date:" + date3 + "\n";
  string signed_headers = "host;x-amz-content-sha256;x-amz-date";
  requestHeaders = curl_slist_sort_insert(requestHeaders, string("host:" + bucket + "." + endpoint).c_str());
  ...
  if(!S3fsCurl::IsPublicBucket()){
    requestHeaders = curl_slist_sort_insert(
          ...
          string("Authorization: AWS4-HMAC-SHA256 Credential=" + AWSAccessKeyId + 
            "/" + date2 + "/cn-north-1/s3/aws4_request, SignedHeaders=" + signed_headers + ", Signature=" +
